...
const solveProblem = (input) => {
    const numbersPerLine = input
        ...
        .map(line =>
            [...line
                .matchAll(new RegExp(`(?=(\\d|${Object.keys(lettersToDigits).join('|')}))`, 'g'))]
                .map(matchAllResult => matchAllResult.at(1))
                .map(numberOrLetters => numberOrLetters in lettersToDigits ? String(lettersToDigits[numberOrLetters]) : numberOrLetters)
        )

    return numbersPerLine.reduce((acc, numbers) =>
        acc + Number(numbers.at(0) + numbers.at(-1))
    , 0);
}
