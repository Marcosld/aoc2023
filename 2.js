...
const solve1 = (input) => {
  ...
  return validGames.reduce((acc, gameNum) => acc + gameNum, 0);
};
...
const solve2 = (input) => {
  ...
  const maxesByGame = games.map((game) =>
    game.reduce(
      (maxesByColor, subset) => {
        Object.entries(subset).forEach(
          ([color, qty]) =>
            (maxesByColor[color] = Math.max(qty, maxesByColor[color])),
        );
        return maxesByColor;
      },
      {
